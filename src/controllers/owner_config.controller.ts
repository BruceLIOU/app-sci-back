...
import { reloadDailyAlerts, startCron } from "../services/cron-manager.service";
import { testSmtpConnection } from "../services/email.service";
import { decrypt, encrypt, isEncrypted } from "../utils/crypto.util";
...
function decryptConfig(config: any): any {
	...
	plain.owner_profile_type = normalizeOwnerProfileType(
		plain.owner_profile_type,
	);
...
function buildAddressFromSeat(seat: any): string {
	if (!seat) return "";
	const parts = [seat.numero_voie, seat.type_voie, seat.libelle_voie].filter(
		Boolean,
	);
...
exports.lookupSiret = async (req: Request, res: Response) => {
	try {
		...
		if (siret.length !== 14) {
			return res
				.status(400)
				.json({ message: "SIRET invalide: 14 chiffres attendus." });
		}
		...
		if (!response.ok) {
			return res
				.status(502)
				.json({ message: "Service SIRET indisponible, reessayez plus tard." });
		}
		...
		if (!first) {
			return res
				.status(404)
				.json({ message: "Aucune entreprise trouvee pour ce SIRET." });
		}

		const seat = first.siege || first.matching_etablissements?.[0] || null;
		const companyName =
			first.nom_complet || first.nom_raison_sociale || first.denomination || "";
		const legalForm =
			first.forme_juridique || first.forme_juridique_libelle || "";
		...
		return res
			.status(500)
			.json({ message: e.message || "Erreur lookup SIRET." });
	}
};
...
exports.testEmail = async (req: Request, res: Response) => {
	try {
		...
		if (!userEmail) {
			return res
				.status(400)
				.json({ message: "Email utilisateur introuvable pour le test SMTP." });
		}
		...
		return res
			.status(500)
			.json({ message: e.message || "Echec du test SMTP." });
	}
};
...
exports.update = async (req: Request, res: Response) => {
	try {
		...
		if (fields.owner_profile_type !== undefined) {
			fields.owner_profile_type = normalizeOwnerProfileType(
				fields.owner_profile_type,
			);
		}
		...
		// Recharger le cron Matera si le schedule ou l'activation ont changé
		const schedule = config.charge_cron_schedule || "0 8 * * *";
		const enabled = config.charge_cron_enabled !== false;
		startCron(schedule, enabled);
		// Recharger le cron des alertes si payment_reminder_enabled a changé
		const remindersEnabled = config.payment_reminder_enabled !== false;
		reloadDailyAlerts(remindersEnabled);
		res.json(decryptConfig(config));
	} catch (e: any) {
		res.status(500).json({ message: e.message });
	}
