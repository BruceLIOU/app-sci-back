...
async function saveDocument(
	...
	const { secure_url, bytes } = await uploadPdfToCloudinary(
		buffer,
		`landlords/pdf/${entity_type}`,
		filename,
	);
...
const quittanceInclude = [
	{
		model: db.Property,
		attributes: ["id", "type", "address", "zipcode", "city", "area"],
	},
	{
		model: db.Tenant,
		attributes: ["id", "civility", "firstname", "lastname", "email", "mobile"],
...
exports.bail = async (req: Request, res: Response) => {
	try {
		const lease = await db.Lease.findByPk(req.params.lease_id, {
		...
		const filename = slug(
			`bail_${property?.city || "bien"}_${tenant?.lastname || "locataire"}_${lease.start_date || "date"}`,
		);
		const title = `Bail – ${property?.city || ""} – ${tenant ? `${tenant.lastname} ${tenant.firstname}` : ""} – ${lease.start_date || ""}`;

		// Attacher au bail
		const docLease = await saveDocument(buffer, {
			title,
			category: "bail",
			entity_type: "lease",
			entity_id: lease.id,
		...
		// Mémoriser l'URL et la date de génération sur le bail
		await lease.update({
			pdf_url: docLease.file_url,
...
exports.quittance = async (req: Request, res: Response) => {
	try {
		const quittance = await db.Quittance.findByPk(req.params.quittance_id, {
			include: quittanceInclude,
		});
		if (!quittance)
			return res.status(404).json({ message: "Quittance introuvable." });
		...
		const filename = slug(
			`quittance_${quittance.period || "periode"}_${tenant?.lastname || "locataire"}`,
		);
		const title = `Quittance – ${quittance.period || ""} – ${tenant ? `${tenant.lastname} ${tenant.firstname}` : ""}`;

		const docQuittance = await saveDocument(buffer, {
			title,
			category: "quittance",
		...
		res.status(201).json({
			message: "PDF généré.",
			document: docQuittance,
			file_url: docQuittance.file_url,
		});
	} catch (e: any) {
		console.error(e);
		res.status(500).json({ message: e.message });
...
exports.etatDesLieux = async (req: Request, res: Response) => {
	try {
		const inspection = await db.Inspection.findByPk(req.params.inspection_id, {
			include: inspectionInclude,
		});
		if (!inspection)
			return res.status(404).json({ message: "État des lieux introuvable." });

		const property = inspection.Property;
		const tenant = inspection.Tenant;
		const landlord = await getLandlord();

		const buffer = await generateEtatDesLieuxPdf(
			inspection,
			property,
			tenant,
			landlord,
		...
		const filename = slug(
			`edl_${typeLabel}_${property?.city || "bien"}_${tenant?.lastname || "locataire"}_${inspection.date || "date"}`,
		);
		const title = `État des lieux d'${typeLabel} – ${property?.city || ""} – ${inspection.date || ""}`;

		const docInspection = await saveDocument(buffer, {
			title,
		...
		res.status(201).json({
			...
			file_url: docInspection.file_url,
		});
	} catch (e: any) {
		console.error(e);
		res.status(500).json({ message: e.message });
...
exports.attestation = async (req: Request, res: Response) => {
	try {
		...
		const buffer = await generateAttestationLoyerPdf(
			lease,
			property,
			tenant,
			landlord,
		...
		const filename = slug(
			`attestation_loyer_${tenant?.lastname || "locataire"}_${new Date().toISOString().slice(0, 7)}`,
		);
		...
		res.status(201).json({
			message: "PDF généré.",
			document: docTenant ?? docLease,
			file_url: (docTenant ?? docLease).file_url,
		});
	} catch (e: any) {
		console.error(e);
		res.status(500).json({ message: e.message });
...
exports.emailBail = async (req: Request, res: Response) => {
	try {
		...
		if (!tenant?.email)
			return res
				.status(400)
				.json({ message: "Le locataire n'a pas d'adresse email." });
		const property = lease.Property;
		const landlord = await getLandlord();
		const buffer = await generateBailPdf(lease, property, tenant, landlord);
...
exports.emailQuittance = async (req: Request, res: Response) => {
	try {
        ...
        <p>Montant total acquitté : <strong>${Number.parseFloat(quittance.total_amount).toFixed(2)} €</strong></p>
        <p>Cordialement,<br><strong>${appName}</strong></p>
      </div>`,
			{ filename, content: buffer },
		);
...
exports.previewBail = async (req: Request, res: Response) => {
	try {
		...
		if (!lease) return res.status(404).json({ message: "Bail introuvable." });
		const landlord = await getLandlord();
		const buffer = await generateBailPdf(
			lease,
			lease.Property,
			lease.Tenant,
			landlord,
...
exports.declaration2072 = async (req: Request, res: Response) => {
	try {
		...
		if (ownerConfig?.owner_profile_type !== "SCI") {
			return res
				.status(403)
				.json({ message: "La déclaration 2072 est réservée au profil SCI." });
		}

		const [payments, charges, associates, properties, landlord] =
			await Promise.all([
				db.Payment.findAll({
					where: { status: "paid" },
					include: [
						{
							model: db.Tenant,
							attributes: ["id", "civility", "firstname", "lastname"],
						},
						{
							model: db.Property,
							attributes: ["id", "type", "address", "zipcode", "city", "area"],
		...
		const totalRevenues: number = yearPayments.reduce(
			(s: number, p: any) => s + Number.parseFloat(p.amount || 0),
			0,
		);

		const yearCharges = charges.filter((c: any) =>
			(c.date || "").startsWith(yearStr),
		);
		const totalCharges: number = yearCharges.reduce(
			(s: number, c: any) => s + Number.parseFloat(c.amount || 0),
			0,
		);
		...
		const byProperty = properties.map((p: any) => {
			...
			const rev: number = yearPayments
				...
				.reduce(
					(s: number, pay: any) => s + Number.parseFloat(pay.amount || 0),
					0,
				);
			const propCharges = yearCharges.filter(
				(c: any) => Number(c.property_id) === pid,
			);
			const chg: number = propCharges.reduce(
				(s: number, c: any) => s + Number.parseFloat(c.amount || 0),
				0,
			);
			return {
