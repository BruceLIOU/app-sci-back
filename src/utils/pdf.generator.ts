...
function landlordManagerName(l: LandlordInfo): string {
	return (
		[l.manager_civility, l.manager_firstname, l.manager_lastname]
			.filter(Boolean)
			.join(" ") || ""
	);
}
...
function section(doc: InstanceType<typeof PDFDocument>, title: string): void {
	// Guard: ensure enough room for the header bar + at least one row before auto-paginating
...
function row(
	...
	// Guard: ensure at least one row height is available before capturing y
...
function signatureBlock(
	...
	if (rightLabel) {
		doc
			.moveTo(doc.page.width / 2 + 20, pageY + 30)
			.lineTo(doc.page.width - 46, pageY + 30)
			.strokeColor(LINE)
			.lineWidth(0.5)
			.stroke();
		doc
			...
			.fontSize(8)
			.font("Helvetica")
			.text(rightLabel, doc.page.width / 2 + 20, pageY + 35, {
				width: 200,
				lineBreak: false,
			});
	}
...
export async function generateBailPdf(
	lease: any,
	property: any,
	tenant: any,
	...
	const doc = new PDFDocument({
		margin: 40,
		size: "A4",
		info: { Title: "Contrat de bail", Author: "Pilotage Immo" },
	});
	...
	if (landlord?.name) {
		...
		const addr = [landlord.address, landlord.zipcode, landlord.city]
			.filter(Boolean)
			.join(", ");
		...
		paragraph(
			doc,
			"(Veuillez compléter les informations du bailleur dans Paramètres)",
		);
	}
	...
	if (tenant) {
		...
		if (tenant.previous_address) {
			row(
				doc,
				"Domicile précédent",
				[tenant.previous_address, tenant.previous_zipcode, tenant.previous_city]
					.filter(Boolean)
					.join(" "),
			);
		}
	...
	if (property.rooms) {
		try {
			...
			const roomList = rooms
				.map((r: any) => `${r.count > 1 ? `${r.count}× ` : ""}${r.type}`)
				.join(", ");
	...
	paragraph(
		doc,
		"Le logement est loué à usage d'habitation principale (et mixte professionnel le cas échéant). " +
	...
	paragraph(
		doc,
		"Le logement est destiné exclusivement à l'usage d'habitation principale du locataire et de sa famille. " +
	...
	row(
		...
		lease.end_date
			...
			: `Reconduction tacite (${dureeLabel[lease.type] || "—"})`,
	);
	row(doc, "Durée légale", dureeLabel[lease.type] || "—");
	row(doc, "Préavis", `${lease.notice_period || 3} mois`);
	paragraph(
		doc,
		`Le contrat est conclu pour une durée de ${dureeLabel[lease.type] || "—"} à compter de la date de prise d'effet. Il se renouvelle par tacite reconduction sauf congé donné dans les formes et délais légaux.`,
	);
	...
	row(
		...
		fmt(
			Number.parseFloat(lease.rent_amount || 0) +
				Number.parseFloat(lease.charges_amount || 0),
		),
		true,
	);
	...
	row(
		...
		"Paiement mensuel à terme échu, avant le 5 de chaque mois",
	);
	paragraph(
		doc,
		"Le loyer sera révisé chaque année à la date anniversaire du contrat selon l'Indice de Référence des Loyers (IRL) " +
	...
	paragraph(
		doc,
		"Les charges récupérables sont celles définies par le décret n°87-713 du 26 août 1987. " +
	...
	paragraph(
		doc,
		`Le dépôt de garantie s'élève à ${fmt(lease.deposit_amount)}. Il sera restitué dans un délai d'un mois si l'état des lieux de sortie est conforme à l'état des lieux d'entrée, ou dans un délai de deux mois dans le cas contraire, conformément à l'article 22 de la loi du 6 juillet 1989.`,
	);
	...
	paragraph(
		doc,
		"Le locataire doit entretenir le logement en bon état et effectuer les réparations locatives définies " +
	...
	paragraph(
		doc,
		"Le locataire est tenu de s'assurer contre les risques locatifs (incendie, dégâts des eaux, etc.) " +
	...
	paragraph(
		doc,
		"Il est expressément convenu qu'à défaut de paiement du loyer ou des charges aux termes convenus, " +
	...
	paragraph(
		doc,
		"Les informations recueillies font l'objet d'un traitement informatique destiné à la gestion locative. " +
	...
	row(
		doc,
		"Lieu et date",
		`${property.city || "—"}, le ${new Date().toLocaleDateString("fr-FR")}`,
	);
	signatureBlock(
		doc,
...
export async function generateQuittancePdf(
	...
	const doc = new PDFDocument({
		margin: 40,
		size: "A4",
		info: { Title: "Quittance de loyer", Author: "Pilotage Immo" },
	});
	...
	if (quittance.number) {
		doc
			...
			.fontSize(9)
			.font("Helvetica")
			.text(`N° ${quittance.number}`, pageW - 165, 88, {
				width: 125,
				align: "right",
				lineBreak: false,
			});
	...
	if (landlord?.name) {
		...
		doc
			.fillColor("#000000")
			.font("Helvetica-Bold")
			.text(landlordName, 46, bY, { width: colW - 10, lineBreak: false });
		bY += 14;
		const mgr = landlordManagerName(landlord);
		if (mgr) {
			doc
		...
		doc.fillColor("#000000");
		if (landlord.address) {
			doc.text(landlord.address, 46, bY, {
				width: colW - 10,
				lineBreak: false,
			});
			bY += 12;
		}
		const cityLine = [landlord.zipcode, landlord.city]
			.filter(Boolean)
			.join(" ");
		if (cityLine) {
			doc.text(cityLine, 46, bY, { width: colW - 10, lineBreak: false });
			bY += 12;
		}
		if (landlord.siret) {
			doc
				.fillColor(GRAY)
				.fontSize(8)
				.text(`SIRET : ${landlord.siret}`, 46, bY, {
					width: colW - 10,
					lineBreak: false,
				});
		}
		...
		doc
			.fillColor(GRAY)
			.font("Helvetica")
			.text("(à compléter dans Paramètres)", 46, bY, {
				width: colW - 10,
				lineBreak: false,
			});
	}
	...
	doc
		...
		.fontSize(9)
		.font("Helvetica-Bold")
		.text(tenantFullName(tenant), rightX + 6, boxY + 22, {
			width: colW - 10,
			lineBreak: false,
		});
	if (property?.address) {
		doc
			.fillColor(GRAY)
			.font("Helvetica")
			.text(property.address, rightX + 6, boxY + 36, {
				width: colW - 10,
				lineBreak: false,
			});
		const tenantCityLine = [property.zipcode, property.city]
			.filter(Boolean)
			.join(" ");
		if (tenantCityLine) {
			doc.text(tenantCityLine, rightX + 6, boxY + 49, {
				width: colW - 10,
				lineBreak: false,
			});
		}
	...
	doc
		.fillColor("#000000")
		.font("Helvetica-Bold")
		.text(fmtDate(quittance.issue_date), pageW / 2 + 96, detY, {
			lineBreak: false,
		});
	doc.y = detY + 16;
	...
	doc
		...
		.fontSize(11)
		.font("Helvetica-Bold")
		.text(fmt(quittance.total_amount), 56, totalBoxY + 8, {
			width: pageW - 112,
			align: "right",
			lineBreak: false,
		});
	...
	paragraph(
		doc,
		`Je soussigné(e), ${bailleurName}, bailleur, déclare avoir reçu de ${tenantFullName(tenant)}, locataire du logement situé ${propertyFullAddress(property)}, la somme de ${fmt(quittance.total_amount)} au titre du loyer et des charges du mois de ${quittance.period || "—"}, et lui en donne bonne et valable quittance, sous réserve de tous droits.`,
	);
	...
	row(
		doc,
		"Fait à",
		`${landlord?.city || property?.city || "—"}, le ${new Date().toLocaleDateString("fr-FR")}`,
	);
	signatureBlock(doc, "Signature du bailleur");

	drawFooter(
		doc,
		"Quittance délivrée gratuitement – Loi n°89-462 du 6 juillet 1989",
	);
	return toBuffer(doc);
}
...
export async function generateEtatDesLieuxPdf(
	...
	const doc = new PDFDocument({
		...
		size: "A4",
		info: {
			Title: `État des lieux d'${typeLabel[inspection.type]?.toLowerCase() || "entrée"}`,
			Author: "Pilotage Immo",
		},
	});
	drawHeader(
		doc,
		`ÉTAT DES LIEUX D'${typeLabel[inspection.type] || "ENTRÉE"}`,
	...
	row(
		doc,
		"Type",
		`État des lieux d'${inspection.type === "entree" ? "entrée" : "sortie"}`,
	);
	...
	row(
		doc,
		"Bailleur / Mandataire",
		landlord?.name ? landlordDisplayName(landlord) : "(à compléter)",
	);
	...
	paragraph(
		doc,
		"Nombre de clés remises : ______  |  Télécommande(s) : ______  |  Digicode : ______",
	);
	paragraph(
		doc,
		"Relevés des compteurs — Électricité : ________  |  Eau froide : ________  |  Eau chaude : ________  |  Gaz : ________",
	);

	// Pièces du bien (property.rooms = [{type, count, area}])
	let propRooms: any[] = [];
	try {
		propRooms = property?.rooms ? JSON.parse(property.rooms) : [];
	} catch {}
	...
	const drawRoomBlock = (
		...
		for (const [left, right] of gridItems) {
			const gY = doc.y;
			// Colonne gauche
			doc
				.fillColor(GRAY)
			...
			if (right) {
				doc
					...
					.fontSize(7.5)
					.font("Helvetica")
					.text(`${right} :`, 45 + colW + 4, gY + 2, {
						width: 85,
						lineBreak: false,
					});
				doc
					.moveTo(45 + colW + 95, gY + 10)
					.lineTo(pageW - 45, gY + 10)
					.strokeColor(LINE)
					.lineWidth(0.3)
					.stroke();
			}
	...
	if (propRooms.length === 0 && legacyRooms.length === 0) {
		...
		for (const pRoom of propRooms) {
			const numRooms =
				typeof pRoom.count === "number" && pRoom.count > 1 ? pRoom.count : 1;
	...
	section(
		...
		inspection.type === "sortie"
			? "VII. Comparaison entrée / sortie"
			: "VII. Réserves",
	);
	if (inspection.type === "sortie") {
		paragraph(
			doc,
			"Dégradations constatées par rapport à l'état des lieux d'entrée (hors usure normale) :",
		);
		doc.moveDown(0.5);
		for (let i = 0; i < 4; i++) {
			doc
		...
		paragraph(
			doc,
			"Réserves émises par le locataire (à compléter dans les 10 jours suivant l'entrée dans les lieux) :",
		);
		doc.moveDown(0.5);
		for (let i = 0; i < 3; i++) {
			doc
	...
	paragraph(
		doc,
		"Les parties reconnaissent que le présent état des lieux a été établi contradictoirement.",
	);
	row(
		doc,
		"Lieu et date",
		`${property.city || "—"}, le ${fmtDate(inspection.date)}`,
	);
	signatureBlock(
		doc,
		"Signature du bailleur\n(ou mandataire)",
...
export async function generateAttestationLoyerPdf(
	lease: any,
	property: any,
	tenant: any,
	landlord?: LandlordInfo,
	...
	const doc = new PDFDocument({
		margin: 40,
		size: "A4",
		info: { Title: "Attestation de loyer", Author: "Pilotage Immo" },
	});
	drawHeader(
		doc,
		"ATTESTATION DE LOYER",
		"Document établi à la demande du locataire",
	);
	...
	const landlordNameAttest = landlord?.name
		? landlordDisplayName(landlord)
		: "(le bailleur)";
	paragraph(
		doc,
		`Je soussigné(e), ${landlordNameAttest}, bailleur du logement situé ${propertyFullAddress(property)}, atteste que ${tenantFullName(tenant)} est locataire de ce logement depuis le ${fmtDate(lease.start_date)} et s'acquitte d'un loyer mensuel de ${fmt(Number.parseFloat(lease.rent_amount || 0) + Number.parseFloat(lease.charges_amount || 0))} charges comprises. Cette attestation est établie pour servir et valoir ce que de droit, notamment dans le cadre d'une demande d'aide au logement (APL/ALS/ALF).`,
	);
...
export async function generateDeclaration2072Pdf(
	data: Declaration2072Data,
): Promise<Buffer> {
	const {
		year,
		landlord,
	...
	const doc = new PDFDocument({
		margin: 40,
		size: "A4",
		info: { Title: `Déclaration 2072-S – ${year}`, Author: "Pilotage Immo" },
	});
	const pageW = doc.page.width;
	...
	if (landlord?.name) {
		...
		paragraph(
			doc,
			"⚠ Informations SCI incomplètes — veuillez renseigner les Paramètres de l'application.",
		);
	}
	...
	const cols = [
		{
			...
			value: fmt(totalRevenues),
			color: "#27ae60",
		},
		{
			label: "Charges déductibles totales",
			value: fmt(totalCharges),
			color: "#c0392b",
		},
		{
			label: `Résultat net (${netResult >= 0 ? "bénéfice" : "déficit"})`,
			value: fmt(Math.abs(netResult)),
			color: netResult >= 0 ? "#2980b9" : "#e67e22",
		},
	];
	for (const [i, col] of cols.entries()) {
		const x = 40 + i * colW3;
		doc
			.rect(x, boxY2, colW3 - 4, 52)
		...
		doc
			...
			.fontSize(7.5)
			.font("Helvetica-Bold")
			.text(col.label.toUpperCase(), x + 4, boxY2 + 4, {
				width: colW3 - 12,
				lineBreak: false,
			});
		doc
	...
	section(
		doc,
		"CADRE III — Immeubles donnés en location (art. 46 C ann. III CGI)",
	);
	...
	if (byProperty.length === 0) {
		...
		for (const [idx, p] of byProperty.entries()) {
			...
			doc
				.fillColor("#000000")
				.text(fmt(p.revenues), colsP[2].x + 2, numY, {
					width: colsP[2].w - 4,
					align: "right",
					lineBreak: false,
				})
				.text(fmt(p.charges), colsP[3].x + 2, numY, {
					width: colsP[3].w - 4,
					align: "right",
					lineBreak: false,
				});
			doc
				.fillColor(netColor)
				.font("Helvetica-Bold")
				.text(`${p.net >= 0 ? "+" : ""}${fmt(p.net)}`, colsP[4].x + 2, numY, {
					width: colsP[4].w - 4,
					align: "right",
					lineBreak: false,
				});
		...
		doc
			...
			.fontSize(8.5)
			.font("Helvetica-Bold")
			.text("TOTAUX", colsP[0].x + 2, tY, {
				width: colsP[0].w + colsP[1].w - 4,
				lineBreak: false,
			})
			.text(fmt(totalRevenues), colsP[2].x + 2, tY, {
				width: colsP[2].w - 4,
				align: "right",
				lineBreak: false,
			})
			.text(fmt(totalCharges), colsP[3].x + 2, tY, {
				width: colsP[3].w - 4,
				align: "right",
				lineBreak: false,
			})
			.text(
				...
				colsP[4].x + 2,
				tY,
				{ width: colsP[4].w - 4, align: "right", lineBreak: false },
			);
		doc.y = tY + 18;
		doc.fillColor("#000000");
	}
	...
	if (allChargeLines.length > 0) {
		...
		for (const c of chCols) {
			doc
				.fillColor(BLUE)
				.fontSize(7.5)
				.font("Helvetica-Bold")
				.text(c.label, c.x + 2, chThY + 4, {
					width: c.w - 4,
					lineBreak: false,
				});
		}
		...
		for (const [idx, c] of allChargeLines.entries()) {
			...
			doc
				...
				.fontSize(8)
				.font("Helvetica")
				.text(c.propLabel, chCols[0].x + 2, rY + 2, {
					width: chCols[0].w - 4,
					lineBreak: false,
				})
				.text(c.type || "—", chCols[1].x + 2, rY + 2, {
					width: chCols[1].w - 4,
					lineBreak: false,
				})
				.text(c.description || "—", chCols[2].x + 2, rY + 2, {
					width: chCols[2].w - 4,
					lineBreak: false,
				})
				.text(c.frequency || "—", chCols[3].x + 2, rY + 2, {
					width: chCols[3].w - 4,
					lineBreak: false,
				});
			doc
				.fillColor("#000000")
				.font("Helvetica-Bold")
				.text(fmt(c.annualAmount), chCols[4].x + 2, rY + 2, {
					width: chCols[4].w - 4,
					align: "right",
					lineBreak: false,
				});
		...
		doc
			...
			.fontSize(8.5)
			.font("Helvetica-Bold")
			.text("TOTAL CHARGES DÉDUCTIBLES", 42, ctY, {
				width: 300,
				lineBreak: false,
			})
			.text(fmt(totalCharges), chCols[4].x + 2, ctY, {
				width: chCols[4].w - 4,
				align: "right",
				lineBreak: false,
			});
	...
	section(
		doc,
		"CADRE V — Répartition du résultat entre associés (art. 8 et 60 CGI)",
	);
	...
	if (byAssociate.length > 0) {
		...
		for (const c of asCols) {
			doc
				.fillColor("white")
				.fontSize(7.5)
				.font("Helvetica-Bold")
				.text(c.label, c.x + 2, asThY + 4, {
					width: c.w - 4,
					lineBreak: false,
				});
		}
		...
		for (const [idx, a] of byAssociate.entries()) {
			if (doc.y > doc.page.height - 80) {
				doc.addPage();
				doc.y = 50;
			}
			...
			const bg = idx % 2 === 0 ? "#f9fafb" : "white";
			doc.rect(40, rY - 2, pageW - 80, 16).fill(bg);
			const fullName = [a.civility, a.firstname, a.lastname]
				.filter(Boolean)
				.join(" ");
			...
			doc
				...
				.fontSize(8)
				.font("Helvetica-Bold")
				.text(fullName, asCols[0].x + 2, rY + 2, {
					width: asCols[0].w - 4,
					lineBreak: false,
				});
			doc
				.font("Helvetica")
				.fillColor(GRAY)
				.text(a.role || "—", asCols[1].x + 2, rY + 2, {
					width: asCols[1].w - 4,
					lineBreak: false,
				})
				.text(`${Number(a.shares).toFixed(2)} %`, asCols[2].x + 2, rY + 2, {
					width: asCols[2].w - 4,
					align: "right",
					lineBreak: false,
				});
			doc
				.fillColor(fc)
				.font("Helvetica-Bold")
				.text(fmt(Math.abs(a.allocated)), asCols[3].x + 2, rY + 2, {
					width: asCols[3].w - 4,
					align: "right",
					lineBreak: false,
				});
	...
	if (allPayments.length === 0) {
		...
		for (const c of pyCols) {
			doc
				.fillColor("white")
				.fontSize(7.5)
				.font("Helvetica-Bold")
				.text(c.label, c.x + 2, pyThY + 4, {
					width: c.w - 4,
					lineBreak: false,
				});
		}
		...
		let pageTotal = 0;
		for (const [idx, p] of allPayments.entries()) {
			if (doc.y > doc.page.height - 80) {
				doc.addPage();
				doc.y = 50;
			}
			...
			doc
				...
				.fontSize(8)
				.font("Helvetica")
				.text(p.propLabel, pyCols[0].x + 2, rY + 2, {
					width: pyCols[0].w - 4,
					lineBreak: false,
				})
				.text(p.tenantName, pyCols[1].x + 2, rY + 2, {
					width: pyCols[1].w - 4,
					lineBreak: false,
				})
				.text(p.month || "—", pyCols[2].x + 2, rY + 2, {
					width: pyCols[2].w - 4,
					lineBreak: false,
				})
				.text(fmtDate(p.paidDate), pyCols[3].x + 2, rY + 2, {
					width: pyCols[3].w - 4,
					lineBreak: false,
				});
			doc
				.fillColor("#000000")
				.font("Helvetica-Bold")
				.text(fmt(p.amount), pyCols[4].x + 2, rY + 2, {
					width: pyCols[4].w - 4,
					align: "right",
					lineBreak: false,
				});
		...
		doc
			...
			.text(fmt(totalRevenues), pyCols[4].x + 2, pyTotY, {
				width: pyCols[4].w - 4,
				align: "right",
				lineBreak: false,
			});
	...
	paragraph(
		doc,
		`Je soussigné(e), ${gerant}, gérant(e) de la société ${landlordName}, certifie l'exactitude des renseignements portés sur la présente déclaration et m'engage à tenir à la disposition de l'Administration fiscale tout document justificatif permettant de vérifier les éléments déclarés, conformément aux dispositions des articles 46 C et 46 D de l'annexe III au CGI.`,
	);
	paragraph(
		doc,
	...
	drawFooter(
		doc,
		`Document établi conformément au Cerfa n°2072-S — Exercice ${year}`,
	);
	return toBuffer(doc);
}
